...
const AnalysisScreen: React.FC = () => {
  ...
  const loadData = async () => {
    const [c, comp, scen] = await Promise.all([fetchCandidates(), fetchComparisonData(), fetchSecondRoundScenarios()]);
    setCandidates(c);
    setComparison(comp);
    setScenarios(scen);
    setLoading(false);
  ...
  if (loading) return (
    <View style={styles.centerContainer}>
      ...
      <Text style={styles.loadingText}>Cargando analisis...</Text>
    </View>
  );
  ...
  const leftCandidates = candidates.filter(c => c.Tendencia_Politica === 'Izquierda').length;
  const centerCandidates = candidates.filter(c => c.Tendencia_Politica === 'Centro').length;
  const rightCandidates = candidates.filter(c => c.Tendencia_Politica === 'Derecha').length;
  ...
  return (
    <ScrollView style={styles.container} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#7c3aed']} />}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Panel de Analisis</Text>
        <Text style={styles.headerSubtitle}>Estadisticas y comparaciones electorales</Text>
      </View>
      ...
      {activeTab === 'general' && (
        <>
          ...
          <View style={styles.chartCard}>
            <Text style={styles.sectionTitle}>Distribucion por Tendencia</Text>
            <View style={styles.tendencyRow}>
              <View style={[styles.tendencyBadge, { backgroundColor: '#fee2e2' }]}>
                <Text style={{ color: '#991b1b', fontWeight: 'bold' }}>Izquierda: {leftCandidates}</Text>
              </View>
              <View style={[styles.tendencyBadge, { backgroundColor: '#dbeafe' }]}>
                <Text style={{ color: '#1e40af', fontWeight: 'bold' }}>Centro: {centerCandidates}</Text>
              </View>
              <View style={[styles.tendencyBadge, { backgroundColor: '#dcfce7' }]}>
                <Text style={{ color: '#166534', fontWeight: 'bold' }}>Derecha: {rightCandidates}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.chartCard}>
            ...
            {[...candidates].sort((a, b) => b.Favorabilidad - a.Favorabilidad).slice(0, 5).map((c, i) => (
              <View key={i} style={styles.favRow}>
                ...
                <View style={styles.favBarContainer}>
                  <View style={[styles.favBar, { width: c.Favorabilidad + '%' }]} />
                </View>
      ...
      {activeTab === 'comparison' && (
        <View style={styles.chartCard}>
          <Text style={styles.sectionTitle}>Comparacion 2022 vs 2026</Text>
          {comparison.length > 0 ? comparison.map((item, i) => (
      ...
      {activeTab === 'scenarios' && (
        <View style={styles.chartCard}>
          ...
          {scenarios.length > 0 ? scenarios.map((scenario, i) => (
...
const styles = StyleSheet.create({
  ...
  tabText: { fontSize: 11, color: '#6b7280', marginLeft: 4 },
  ...
  tendencyRow: { flexDirection: 'row', justifyContent: 'space-around', flexWrap: 'wrap' },
  tendencyBadge: { paddingHorizontal: 16, paddingVertical: 12, borderRadius: 12, margin: 4 },
  ...
  favBar: { height: '100%', backgroundColor: '#22c55e', borderRadius: 8 },
  ...
  scenarioTitle: { fontSize: 14, fontWeight: '600', color: '#7c3aed', marginBottom: 12 },
